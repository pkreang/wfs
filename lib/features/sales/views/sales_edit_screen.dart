...
class SalesEditScreen extends ConsumerStatefulWidget {
  final String userID;
  const SalesEditScreen({super.key, required this.userID});
...
class _SalesEditScreenState extends ConsumerState<SalesEditScreen> {
  ...
  final _saleProfileImageController = TextEditingController();
  ...
  void dispose() {
    ...
    _saleProfileImageController.dispose();
    super.dispose();
  }
  ...
  void _handleSave() async {
    ...
    final result = await ref.read(salesEditProvider(widget.userID).notifier).updateSales();
    if (!result) {
      final errMsg = ref.read(salesEditProvider(widget.userID)).errorMessage;
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(salesEditProvider(widget.userID));
        ...
        GestureDetector(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const AppText(label: 'Edit User', fontSize: 17, fontWeight: FontWeight.w600),
              actions: [
                TextButton(
  ...
  Widget buildContent(User user) {
    if (!isInit) {
      ...
      _passwordController.text = user.hashedPassword ?? '';
            ...
            children: [
              Column(
                children: [
                  FormInfoTile(
                    label: 'FirstName',
                    value: AppTextFormField(controller: _firstNameController, onChanged: (value) => ref.watch(salesEditProvider(widget.userID).notifier).setFirstName(value)),
                    isHideIcon: true,
                  ),
                  FormInfoTile(
                    label: 'LastName',
                    value: AppTextFormField(controller: _lastNameController, onChanged: (value) => ref.watch(salesEditProvider(widget.userID).notifier).setLastName(value)),
                    isHideIcon: true,
                  ),
                  FormInfoTile(
                    label: 'Email',
                    value: AppTextFormField(controller: _emailController, onChanged: (value) => ref.watch(salesEditProvider(widget.userID).notifier).setEmail(value)),
                    isHideIcon: true,
                  ),
                  FormInfoTile(
                    label: 'Phone',
                  ...
                  FormInfoTile(
                    ...
                    onTap: () =>
                        AppSheet.openRoleSheet(context: context, userRoleID: user.userRoleID ?? '', onSelected: (value) => ref.watch(salesEditProvider(widget.userID).notifier).setUserRole(value)),
                  ...
                  if (user.userRole?.isSale ?? false)
                    FormInfoTile(
                      ...
                      onTap: () =>
                          AppSheet.openSupervisorSheet(context: context, userID: user.managerID ?? '', onSelected: (value) => ref.watch(salesEditProvider(widget.userID).notifier).setManager(value)),
